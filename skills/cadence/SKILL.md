...
Keep the libraries separate, including in every search. Classify uploads from the user's description and document purpose. Ask only when the classification materially changes how a source will be used. The bundled ADDIE guide is a working method, not a third evidence library or an institutional mandate. No course content is preloaded. The user-selected ADDIE artifact catalog is bundled in `assets/institution/` and indexed into the institution library when a new course is initialized. If missing institutional evidence is consequential, request it with the option to skip; otherwise record the limitation. After a skip, use generic ADDIE criteria and do not claim institutional conformance.
...
For a new project, read [references/intake.md](references/intake.md) first. Collect the project name, build/review intent, delivery format, learner group, course duration, user role, and source availability through its short intake form before initializing the course or starting ADDIE work. Reuse answers the user already supplied, accept explicit uncertainty, and resume existing projects without repeating intake. Use the saved answers in the scope brief and specialist assignments.

Read [references/artifact-deliverables.md](references/artifact-deliverables.md) for the user-selected institutional catalog. Index that guidance before taking the source snapshot. In Analysis produce the Analysis Summary Report; in Design produce the Course Design Document; in Development produce the applicable named artifacts. Preserve review-only scope when reviewing existing materials. Track each catalog subsection in the progress panel and mark it complete only after saving and checking its supporting content.

Read [references/progress.md](references/progress.md) when starting or resuming curriculum work. Maintain its saved progress markers and, when supported, open its live panel beside the chat. Update phases, actual specialist assignments, waiting decisions, and existing deliverables at meaningful checkpoints. Keep this display factual; progress is not approval or proof of learning effectiveness.
